	...
	{#if loaded}
		...
		{#if collapsible}
			<Collapsible bind:open className="w-full" buttonClassName="w-full" onChange={setOpen}>
				<div class="flex items-center justify-between h-7 w-full pl-3.5 pr-1.5 shrink-0">
					<button
						type="button"
						class="group flex flex-1 h-full items-center gap-1 text-left text-[13px] leading-5 text-gray-400 hover:text-gray-500 dark:text-gray-500 dark:hover:text-gray-400 transition-colors duration-100 {buttonClassName}"
						aria-expanded={open}
